...
void Armordetection::color_detection(int my_color, bool show_mask)  // 后期再加入其他方法
{
    //my_color:0 蓝色 ； 1 红色； 测试需要敌我都是同种颜色，故my_color = enermy_color ;后期修改
//       my_color = 1;
       if(my_color == 1)
       {
//         cout<< "======================识别蓝色======================"<< endl;
         ...
         vector<Mat> splited;
         Mat color,mask;
         int thres_max_color_blue = 50;
         split(_roi,splited);
         cvtColor(_roi,thres_whole,CV_BGR2GRAY);

         threshold(thres_whole,thres_whole,85,255,THRESH_BINARY);
...
//        if(lightContourArea < MIN_LightContourArea){/*cout<<"LIGHT:lightContourArea0"<<lightContourArea<<endl;*/continue;}
        // 灯条面积太小的不要，注意这里可以再分细一点
        // 由于灯条刚刚转过来的时候，面积很小，w/h比也很小，可以结合这两个来筛选
        // 从而保证刚刚转过来的面积很小的灯条不会被筛选掉
        // 面积很小， w/h也要很小才保留
//        if(lightContourArea < LIGHTCONTOUR1 && k > K_LightContourArea1) { /*cout<<"K_LightContourArea1"<<lightContourArea<<endl; cout<<"K_LightContourArea1"<<k<<endl; */  continue;}
//        // 面积更小， w/h也要更小才保留
//        if(lightContourArea < LIGHTCONTOUR2 && k > K_LightContourArea2)  {/*cout<<"K_LightContourArea2"<<lightContourArea<<endl; cout<<"K_LightContourArea2"<<k<<endl; */continue;}

        // 灯条面积太大的不要
//        if(lightContourArea > MAX_LightContourArea) {/*cout<<"MAX_LightContourArea"<<lightContourArea<<endl;*/ continue;}

        // 旋转矩形长宽比太大的灯条不要  // 太胖的不要
        if( k > MAX_LightHighWeigh ) {/*cout<<"MAX_LightHighWeigh "<<k<<endl;*/ continue;}

        // 灯条左右倾斜超过一定角度的不要
        if (rect.size.width == min(rect.size.width, rect.size.height) &&
            fabs(rect.angle) > MAX_LightAngle1 ) {/*cout<<"MAX_LightAngle1"<<fabs(rect.angle)<<endl;*/continue;}
        if (rect.size.width == max(rect.size.width, rect.size.height) &&
            fabs(rect.angle) < MAX_LightAngle2 ) {/*cout<<"MAX_LightAngle2"<<endl;*/ continue;}
...
//           cout<<"NOT_MATCH 灯条的长度"<<"min"<<min(h1, h2)<<"max"<<max(h1, h2)<<endl;
           ...
           if (fabs (top1.y+bottom1.y - top2.y-bottom2.y) <  CENTER_RADIO * (double)(h1+h2)/(4))
           {
//             cout<<"6: "<<fabs (top1.y+bottom1.y - top2.y-bottom2.y)<<"<"<< CENTER_RADIO *(double)(h1+h2)/(4)<<endl;
             temp[i][j] = temp[i][j] + 1; }
//           else
//           {
//               cout<<"NOT MATCH"<<fabs (top1.y+bottom1.y - top2.y-bottom2.y)<<"\t"<<(double)(h1+h2)/(4)<<endl;
//           }
                  ...
                  else
                   {
                        for(size_t m = 0; m < Match_index.size(); m++)
                        {
                            ...
                            if (i == Match_index.at(m) || j == Match_index.at(m))
                            {
                               ...
                               double k_ij = fabs((double)(top1.y-top2.y)/(double)(top1.x-top2.x));
                               ...
                               double k_index = fabs((double)(top1.y-top2.y)/(double)(top1.x-top2.x));
                               vPolygonPoint.clear();
                               vPolygonPoint.push_back(top1);
                               vPolygonPoint.push_back(bottom1);
                               vPolygonPoint.push_back(bottom2);
...
vector<Point> Armordetection::choose_target(vector<vector<Point>>& Match)
{
    ...
    for(size_t i = 0; i<Match.size(); i=i+2)
    {
        ...
        double w_w = max(abs(top1.x-top2.x), abs(bottom1.x-bottom2.x));
        double h_h = max(abs(top1.y-bottom1.y), abs(top2.y-bottom2.y));
//        cout<<"7旋转矩形长宽比:"<<endl;
//        cout<<w_w/h_h<<endl;
...
Rect Armordetection::get_roi(vector<Point>& armor_4point, double roi_scale)
{
   ...
   int xmax =0, xmin = 10000, ymax = 0, ymin = 10000, rw=0, rh=0;  //初始化一下
    ...
    xmin = max(1, (int)(xmin - w*roi_scale*0.5));
    ymin = max(1, (int)(ymin - h*roi_scale));
    //    xmin = max(1, (int)(xmin - 2*(xmax-xmin)*roi_scale));
    //    ymin = max(1, (int)(ymin - 2*(ymax-ymin)*roi_scale));

    rw = min((int)(w*(1+roi_scale)), _img.cols-xmin);
    rw = max(1, rw);
    rh = min((int)(h*(1+2*roi_scale)), _img.rows-ymin);
    rh = max(1, rh);
                     ...
                     bool get_num_image, int num_img_size, bool show_num_img,Numpredict &num_pre)
{
    ...
    if(match.size()>=2)
    {
     ...
     t5 = (double)getTickCount();
    target = this->choose_target(match);  // 打击目标的四个顶点
//    cout<<55555<<endl;
    t6 = (double)getTickCount();
...
//        cout<<"predict num consuming time : "<<(end_num-start_num)/getTickFrequency()<<endl;
    ...
    else   //设置ROI
    {
        ...
        if (_lossFrameK < 20) {
            ...
            if (_lossFrameK> 0)
            {
                roi_scale = 0.01*roi_scale + 0.01 * (double)_lossFrameK;
            }
             ...
             xmin = max(1, (int)(xmin - w*roi_scale));
             int rw = min((int)(w*(1+2*roi_scale)), _img.cols-xmin);
             rw = max(1, rw);
             int rh;
             rh= min((int)(h), _img.rows-ymin);
             rh = max(1, rh);
...
//              rw = max(1, rw);
//              int rh = min((int)(ymax - ymin), _img.rows-ymin);
//              rh = max(1, rh);
